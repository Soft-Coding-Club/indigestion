...
send({t:"reading",text:"MY MOM DRANK HER COFFEE"});
for(const ms of [400,1500,2500,4000]){
  await pg.waitForTimeout(ms);
  const st=await pg.evaluate(()=>({n:letters.length,
    sample:letters.slice(0,4).map(L=>({ch:L.ch,y:Math.round(L.y),x:Math.round(L.x),size:Math.round(L.size),life:+L.life.toFixed(2)})),
    H:H()}));
